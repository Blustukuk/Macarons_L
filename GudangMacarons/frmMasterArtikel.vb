Public Class frmMasterArtikel
    Dim dt As DataTable
    Private Sub btnclose_Click(sender As Object, e As EventArgs) Handles btnclose.Click
        Dispose()
    End Sub

    Private Sub frmMasterArtikel_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Call gantiwarnaform(sender, 99, 205, 218)
        Call frmmasterartikelload()
    End Sub
    ...
    Private Sub tampilkanData(ByVal dt As DataTable)
        If dt.Rows.Count > 0 Then
            ...
            lblkelompokid.Text = CStr(dt.Rows(0).Item("id"))
            txtkode.Text = CStr(dt.Rows(0).Item("kode"))
            cbxBrand.SelectedIndex = cbxBrand.FindStringExact(dt.Rows(0).Item("Brand"))
            txtArtikel.Text = CStr(dt.Rows(0).Item("Artikel"))
        ...
        End If
    End Sub
    Private Sub txtnamasupplier_GotFocus(sender As Object, e As EventArgs) Handles txtArtikel.GotFocus
    ...
    Private Sub btncari_Click(sender As Object, e As EventArgs) Handles btncari.Click
        dt = loadDataArtikel(txtcari.Text)
        tampilkanData(dt)
    End Sub

    Private Sub btntambah_Click(sender As Object, e As EventArgs) Handles btntambah.Click
        Dim namaArtikel, kode As String
        Dim tambah As Integer
        Dim cek As Boolean
        namaArtikel = System.Text.RegularExpressions.Regex.Replace(txtArtikel.Text, "[^a-zA-Z0-9]", " ")
        kode = randomStr(3)
        If txtArtikel.Text = "" Then
            MsgBox("Isi terlebih dahulu nama artikel.", vbCritical, "Macarons")
        Else
            cek = cekArtikel(kode, namaArtikel)
            If cek Then
                tambah = masterartikeltambah(namaArtikel, cbxBrand.SelectedValue, chkbaktif.Checked, kode)
                If tambah <> 0 Then
                ...
                End If
            Else
                MsgBox("Kode atau Nama sudah ada. Silahkan gunakan nama lainnya.", vbOKOnly, "Macarons")
            End If
        End If
    End Sub

    Private Sub btnedit_Click(sender As Object, e As EventArgs) Handles btnedit.Click
        Dim namaArtikel As String
        Dim edit As Integer
        Dim cek As Boolean
        namaArtikel = System.Text.RegularExpressions.Regex.Replace(txtArtikel.Text, "[^a-zA-Z0-9]", " ")
        If lblkelompokid.Text <> "0" Then
            cek = cekArtikel("", namaArtikel)
            If cek Then
                edit = masterartikeledit(lblkelompokid.Text, cbxBrand.SelectedValue, namaArtikel, chkbaktif.Checked, txtkode.Text)
                If edit <> 0 Then
        ...
        If e.ColumnIndex = 5 And e.RowIndex >= 0 Then
            ...
            Else
                ...
                If isAdmin Then
                    ...
                    cek = cekDeleteArtikel(idDel)
                    If cek Then
                        del = masterartikeldel(idDel)
                        If del Then
                        ...
                        End If
                    Else
                        MsgBox("ID Artikel tidak ditemukan.", vbCritical, "Macarons")
                    End If
                Else
    ...
    Private Sub dgvtemp_CellContentDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvtemp.CellContentDoubleClick
        If e.RowIndex >= 0 Then
            Dim idDel As String = dgvtemp.Rows(e.RowIndex).Cells(0).Value.ToString
            Dim dt2 As DataTable = loadDataArtikelID(idDel)
            If dt2.Rows.Count > 0 Then
                lblkelompokid.Text = CStr(dt2.Rows(0).Item("id"))
                txtkode.Text = CStr(dt2.Rows(0).Item("kode"))
                txtArtikel.Text = CStr(dt2.Rows(0).Item("Artikel"))
                ...
                End If
            Else
                MsgBox("Data artikel tidak ada.", vbCritical, "Macarons")
            End If
        End If
    End Sub
End Class